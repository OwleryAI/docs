...
<Tabs>
  <Tab title="Manual entry (the + button)">
    ...
    Then below the pricing section, you define each stop on the lane using the **Route Stop Config** editor. Each stop has:
...
Each row has a ⋮ menu with:\
 Each row has a ⋮ menu with: Each row has a ⋮ menu with:\
Each row has a ⋮ menu with:
